...
(
        ...
        city varchar(25),
        grade int,
        salesman_id int references salesman(salseman_id)
)
insert into
    Customer
values
...
select sm.name as SalesMan,c.cust_name,c.city 
from Salesman  as sm 
...
select  c.Cust_name as Customer_name,c.city as customer_city,sm.name as Salesman,sm.commission 
from Salesman as sm 
inner join customer as c 
on sm.salseman_id=c.salesman_id 
...
select c.customer_id,c.cust_name,c.city as customer_city, c.grade, sm.name as Salesman, sm.city as Salesman_city
from Salesman as sm
inner join Customer as c
on sm.Salseman_id = c.salesman_id order by  c.customer_id